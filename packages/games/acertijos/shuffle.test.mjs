...
describe("buildQuestions sobre el banco real (40 acertijos, reto diario)", () => {
  it("elige exactamente 5 acertijos sin repetir", () => {
    const built = buildQuestions(BANK, 2026071601, 5);
    expect(built).toHaveLength(5);
    expect(new Set(built.map((q) => q.q)).size).toBe(5);
  });

  it("misma seed -> los mismos 5 acertijos (mismo orden) dos veces", () => {
